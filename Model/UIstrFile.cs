...
namespace ESO_Lang_Editor.Model
{
    class UIstrFile
    {
        public string UI_Table { get; set; }
        ...
        public void createDB(Dictionary<string, UIstrFile> Lua_EN, Dictionary<string, UIstrFile> Str_ZH, string TableName)
        {
            var luaData = Lua_EN;
            var strData = Str_ZH;
                    ...
                    foreach (var t in tableName)
                    {
                        ...
                        while (sr.Read())
                        {
                            _LangViewData.Add(new UIstrFile
                            {
                                ...
                                UI_EN = sr.GetString(1),               //游戏内文本Unknown列
                                UI_ZH = sr.GetString(2),                 //游戏内文本Index
                                UI_Version = sr.GetInt32(3),                 //英语原文
                                RowStats = sr.GetInt32(4),                  //汉化文本
        ...
        public string UpdateStrFromEditor(UIstrFile CsvContent)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=" + UIstrDBPath + ";Version=3;"))
            {
                conn.Open();
                ...
                try
                {
                    cmd.CommandText = "UPDATE " + CsvContent.UI_ID
                        + " SET UI_ZH=@UI_ZH,isTranslated=@isTranslated"
                        + " WHERE UI_ID='" + CsvContent.UI_ID + "'";

                    cmd.Parameters.AddRange(new[]
                    {
                        new SQLiteParameter("@UI_ZH", CsvContent.UI_ZH),
                        new SQLiteParameter("@isTranslated", CsvContent.isTranslated)

                    });
                    cmd.ExecuteNonQuery();
                    return CsvContent.UI_ZH + " 更新成功！";

                }
                catch (Exception ex)
                {
                    return "插入数据：" + CsvContent.UI_ZH + " 失败：" + ex.Message;
                    throw new Exception("插入数据：" + CsvContent.UI_ZH + "失败：" + ex.Message);
                }
        ...
        public string CreateDBFileFromLUA(List<UIstrFile> Content)
        {
            ...
            {
                if (!tableID.Contains(table.UI_Table))
                    tableID.Add(table.UI_Table);
            }

            ConnectSQLite();
            CreateTableToStrDB(tableID);
        ...
        public void CreateTableToStrDB(string TableName, string StrDBPath)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=" + StrDBPath + ";Version=3;"))
            {
                conn.Open();
                SQLiteCommand cmd = new SQLiteCommand();
                cmd.Connection = conn;
                try
                {
                    cmd.CommandText = "CREATE TABLE " + TableName + "(@UI_ID string, @UI_EN string, @UI_ZH string, @UI_Version int, @RowStats int, isTranslated int)";
                    cmd.ExecuteNonQuery();
